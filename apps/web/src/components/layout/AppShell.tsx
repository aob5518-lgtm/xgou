  ...
  return (
    <div className="app-surface min-h-screen bg-[var(--background)] pb-24 lg:pb-0">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 border-r border-[var(--border)] bg-[#07090b]/95 p-6 lg:flex lg:flex-col">
        ...
        <p className="text-[10px] leading-5 tracking-[.12em] text-white/30">{isDemo ? 'DEMO ENVIRONMENT' : 'ARC TESTNET'}<br />NO MAINNET ASSETS</p>
      </aside>
      <div className="lg:pl-64">
        <header className="sticky top-0 z-20 flex h-20 items-center justify-between border-b border-[var(--border)] bg-[#050607]/80 px-5 backdrop-blur-xl md:px-8">
          ...
          <div className="hidden items-center gap-3 text-[10px] tracking-[.18em] text-white/45 sm:flex"><span className="size-1.5 rounded-full bg-[var(--success)] shadow-[0_0_10px_var(--success)]" />{isDemo ? 'ARC DEMO · SYSTEM ONLINE' : 'ARC TESTNET · REAL LEDGER'}</div>
          <div className="flex items-center gap-3"><span className="rounded-full border border-white/10 bg-white/[.025] px-2.5 py-1.5 text-[8px] tracking-[.14em] text-white/35">{isDemo ? 'DEMO MODE' : 'TESTNET'}</span><WalletButton /></div>
        </header>
        <main className="mx-auto max-w-[1600px] px-5 py-7 md:px-8 md:py-12">{children}</main>
        <footer className="border-t border-[var(--border)] px-6 py-8 text-center text-[10px] tracking-[.16em] text-white/30">{isDemo ? 'DEMO ENVIRONMENT.' : 'ARC TESTNET.'} NO MAINNET ASSETS OR LIVE TRADING.</footer>
      </div>
